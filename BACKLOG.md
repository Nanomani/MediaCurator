...
- [ ] `[P2]` **Subtitle download** — for files that have no subtitle tracks at all, offer a right-click "Find Subtitles…" action; opens a small dialog that searches a subtitle service (OpenSubtitles or similar) by IMDb ID / title + year / file hash; displays results with language, format (SRT/ASS), and uploader rating; downloading saves the `.srt` file alongside the video (Kodi/Plex-compatible naming: `Movie.Title.2001.en.srt`) and adds a row to the file's stream display showing the external sub
