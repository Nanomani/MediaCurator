...
- [ ] `[P3]` **Evaluate dropping vcpkg — nlohmann/json is a single header** — vcpkg was added for nlohmann/json but that library is header-only and trivially vendored; audit whether any other vcpkg package is actually used; if not, drop vcpkg entirely and vendor nlohmann/json directly under `third_party/` to simplify the build setup and remove the CMakeUserPresets VCPKG_ROOT requirement
